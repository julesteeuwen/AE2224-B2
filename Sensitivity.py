...
def calculate_predicted_complexity(ansps, n):
    for ansp in ansps:
        Horizontal_lst = predict_ansp_TBATS(ansp, 'HORIZ_INTER_HRS' ,n)*0.9
        Vertical_lst = predict_ansp_TBATS(ansp, 'VERTICAL_INTER_HRS' ,n)*0.9
        Speed_lst = predict_ansp_TBATS(ansp, 'SPEED_INTER_HRS' ,n)*0.9
        Horizontal = sum(Horizontal_lst.values)
        Vertical = sum(Vertical_lst.values)
        Speed = sum(Speed_lst.values)
    print(Horizontal)
    cplx_verti = 0
    cplx_hori = 0
    cplx_speed = 0
    return cplx_verti, cplx_hori, cplx_speed, ansps
calculate_predicted_complexity(ansps, n)
...
def plot_change_actual_complexity(file, ansps):
    ...
    cplx_verti, cplx_hori, cplx_speed, ansps = calculate_actual_complexity('Datasets/2017-2019_vertical_sensitivity.csv', ansps)
...
def plot_change_predicted_complexity(file, ansps, n):
    cplx_old, ansps = calculate_actual_complexity(file, ansps)
    cplx_verti, cplx_hori, cplx_speed, ansps = calculate_predicted_complexity(ansps,n)
    change_verti = []
    change_hori = []
    change_speed = []
    for i in range(len(cplx_old)):
...
#plot_change_predicted_complexity(file, ansps, n)
