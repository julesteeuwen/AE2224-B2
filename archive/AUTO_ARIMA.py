...
def auto_ARIMA(dataframe, label, test_size=0.2, plotting = False):
    train, test = train_test_split(dataframe, test_size = test_size, shuffle=False)
    model = auto_arima(x=train.index, y=train[label].values, trace=True, error_action='ignore', suppress_warnings=True, m = 30)
...
#Trend_Decompose(df, "HORIZ_INTER_HRS", plotting =True)S
#auto_ARIMA(df, "VERTIC_INTER_HRS", test_size = 0.2, plotting = True)